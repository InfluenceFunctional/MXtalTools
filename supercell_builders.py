...
from dataset_management.random_crystal_builder import \
    (new_cell_analysis, update_supercell_data, compute_lattice_vector_overlap,
     compute_principal_axes_torch, fast_differentiable_coor_trans_matrix,
     fast_differentiable_ref_to_supercell, clean_cell_output)
...
def override_sg_info(override_sg, dataDims, supercell_data, symmetries_dict):
    ...
    supercell_data.x[:, -dataDims['n crystal generation features']] = 0  # set all crystal features to 0
...
class SupercellBuilder():
    ...
    def build_supercells(self, supercell_data, cell_sample, supercell_size, graph_convolution_cutoff,
        ...
        if ref_data is not None:  # extract parameters directly from the ref_data
            cell_sample_i, ref_final_coords = new_cell_analysis(supercell_data.clone(), self.atom_weights, return_final_coords=True, debug = debug)
            if standardized_sample:
                cell_sample = (cell_sample_i - torch.Tensor(self.dataDims['means'])) / torch.Tensor(self.dataDims['stds'])  # standardize
            else:
        ...
        if skip_cell_cleaning:  # don't clean up
            if standardized_sample:
                destandardized_cell_sample = (cell_sample * torch.Tensor(self.dataDims['stds'])) + torch.Tensor(self.dataDims['means'])  # destandardize
        ...
        disagreements = torch.where(mol_handedness == -1)[0]
        for n in range(len(disagreements)):  # invert molecules which are left-handed
            coords = coords_list[disagreements[n]]
            coords_inv = -(coords - coords.mean(0)) + coords.mean(0)
            coords_list[disagreements[n]] = coords_inv

        # recompute principal axes
        Ip_axes_list = torch.zeros((len(masses_list), 3, 3)).to(T_fc_list.device)
        for i, (masses, coords) in enumerate(zip(masses_list, coords_list)):
            Ip_axes_list[i], _, _ = compute_principal_axes_torch(masses, coords)
        ...
        if debug:
            mol_handedness = torch.sign(torch.mul(Ip_axes_list[:, 0], torch.cross(Ip_axes_list[:, 1], Ip_axes_list[:, 2], dim=1)).sum(1))
            assert mol_handedness.sum() == len(mol_handedness)

        normed_alignment_target_list = torch.stack([torch.eye(3) for n in range(len(coords_list))]).to(coords_list[0].device) # always use the right-handed target
        ...
        # skip this one, since we are now doing a lot of coordinate inversion to enforce handedness
        # if (ref_data is not None) and debug:
        #     for i in range(supercell_data.num_graphs):
