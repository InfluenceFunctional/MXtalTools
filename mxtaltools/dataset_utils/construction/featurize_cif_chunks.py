...
from mxtaltools.dataset_utils.construction.featurization_utils import extract_crystal_data, featurize_molecule, \
    crystal_filter, \
    chunkify_path_list, extract_custom_cif_data, instantiate_crystal, init_zp1_crystals, extract_zp1_pose_info, \
    cocrystal_check, overwrite_conformer_to_zp, check_zp_mol_alignment
...
def process_chunk(chunk, chunk_ind, use_filenames_for_identifiers, protonation_state, max_z_prime):
    ...
    for ind, cif_path in enumerate(tqdm.tqdm(chunk)):
        ...
        for crystal_ind in range(len(reader)):  # one cif file may have many crystals in it
            ...
            try:
                csd_crystal, reduced_crystal = init_reader(crystal_ind, protonation_state, reader)
            ...
            crystal_dict, molecules = extract_crystal_info(cif_path, csd_crystal, identifier, protonation_state,
                                                           rd_mols, reduced_crystal, unit_cell)
            ...
            if len(mols) > 1:
                if not cocrystal_check(mols):
                    ...
                    continue  # we are not set up to handle cocrystals
            ...
            (aunit_centroid, aunit_handedness,
             ...
             is_well_defined, pos) = extract_zp1_pose_info(crystal_dict['unit_cell_coordinates'], z_prime, zp1_crystals)
            ...
            if z_prime > 1:
                # for our current purposes, enforce identical atom ordering and conformers in Z' images
                aligned_batch = overwrite_conformer_to_zp(mols, rebuild_batch, z_prime)
                if not check_zp_mol_alignment(aligned_batch, rebuild_batch):
                    failed_parameterization_counter += 1
                    error_codes.append("Z'>1 image writing failed")
                    continue

                rebuild_batch = aligned_batch  # overwrite

            finished_crystal = instantiate_crystal(aunit_centroid, aunit_handedness,
...
def extract_crystal_info(cif_path, csd_crystal, identifier, protonation_state, rd_mols, reduced_crystal, unit_cell):
    ...
    for i_c, rd_mol in enumerate(rd_mols):  # one crystal may have Z' molecules
        mol_dict = featurize_molecule(reduced_crystal, rd_mol,
                                      component_num=i_c,
    ...
    return crystal_dict, molecules


def init_reader(crystal_ind, protonation_state, reader):
    ...
    return csd_crystal, reduced_crystal
...
def crystal_rebuild_checks(aunit_centroid,
    ...
    uc_build = crystal_batch.unit_cell_pos  # full unit cell
    ...
    T_fc = crystal_batch.T_fc[0]  # fractional → Cartesian (whatever your attr is)
    T_cf = torch.linalg.inv(T_fc)

    frac_orig = uc_orig @ T_cf.T
    frac_build = uc_build @ T_cf.T

    df = frac_orig[:, None, :] - frac_build[None, :, :]  # (N, N, 3)
    df -= torch.round(df)  # minimum image
    dcart = df @ T_fc.T
    distmat = torch.linalg.norm(dcart, dim=-1)
    ...
    single_matches = len(torch.unique(nearest_neighbors)) == len(nearest_neighbors)
    if (nn_dists.amax() < 5e-2) and (nn_dists.mean() < 1e-2) and single_matches:  # every atom matched
        try:
...
if __name__ == '__main__':
    ...
    #                        cifs_path=r"D:\crystal_datasets\acridine\adam_opt",
    #                        chunks_path=r"D:\crystal_datasets\acridine\adam_opt",
    #                        chunk_prefix='',
    ...
    process_cifs_to_chunks(n_chunks=1000,
                           ...
                           chunks_path='D:/crystal_datasets/CSD_prot_featurized_chunks/',
                           chunk_prefix='',
                           use_filenames_for_identifiers=False,
                           target_identifiers=None,
                           filter_by_targets=False,
    ...
    # process_cifs_to_chunks(n_chunks=1,
    #                        cifs_path='D:/crystal_datasets/acridine/canonical_structures',
    #                        chunks_path='D:/crystal_datasets/acridine/canonical_structures',
    #                        chunk_prefix='',
    #                        use_filenames_for_identifiers=False,
    #                        target_identifiers=None,
    #                        filter_by_targets=False,
