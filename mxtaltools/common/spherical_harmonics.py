"""Ripped out of e3nn to avoid installation"""
import torch
import math


def spherical_harmonics(lmax: int, coords: torch.Tensor) -> torch.Tensor:
    """
    assume incoming tensors are in shape [n, 3, k]
    """
    p_coords = coords.permute(0, 2, 1)
    return _spherical_harmonics(lmax, p_coords[..., 0], p_coords[..., 1], p_coords[..., 2]).permute(0, 2, 1)
...
def _spherical_harmonics(lmax: int, x: torch.Tensor, y: torch.Tensor, z: torch.Tensor) -> torch.Tensor:
    ...
    sh_10_1 = (
            (1 / 10) * math.sqrt(21) * sh_9_0 * y + (3 / 20) * math.sqrt(42) * sh_9_1 * z + (3 / 20) * math.sqrt(
        42) * sh_9_17 * x
    )
    sh_10_2 = (
    ...
    sh_10_19 = (
            -3 / 20 * math.sqrt(42) * sh_9_1 * x + (3 / 20) * math.sqrt(42) * sh_9_17 * z + (1 / 10) * math.sqrt(
        21) * sh_9_18 * y
    )
