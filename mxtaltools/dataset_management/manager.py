...
class DataManager:
    ...
    def filter_protons_ROUGH(self):
        ...
        for ind, row in tqdm(self.dataset.iterrows(), total=self.dataset.shape[0]):
            ...
            for key in atom_keys:
                row[key] = np.asarray(row[key])[heavy_atom_inds]
    ...
    def set_tracking_keys(self):
        ...
        # these three necessary to function
    ...
    def process_new_dataset(self, new_dataset_name):
        ...
        if 'qm9' in new_dataset_name.lower():
            ...
            '''delete molecules with only one heavy atom'''
            small_mol_inds = np.argwhere(self.dataset['molecule_num_atoms'] <= 5)[:, 0]
            bad_inds = []
            for ind in small_mol_inds:
                atoms = self.dataset['atom_atomic_numbers'].iloc[ind]
                if np.sum(atoms >= 2) == 1:
                    bad_inds.append(ind)
            self.dataset = delete_from_dataframe(self.dataset, bad_inds)
