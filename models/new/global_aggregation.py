...
class global_aggregation(nn.Module):
    ...
    def forward(self, x, batch, cluster=None, output_dim=None):
        if self.agg_func == 'set2set':
            ...
            # assume the input is nx3 dimensional
            vecs = x.reshape(x.shape[0], x.shape[1] // 3, 3)
            norms = torch.linalg.norm(vecs, dim=-1)
            agg = torch.cat([vecs[batch == bind][norms[batch == bind].argmax(dim=0), torch.arange(torch.sum(batch==bind))] for bind in range(batch[-1] + 1)])
            return agg
        else:
