...
class MikesGraphNet(torch.nn.Module):
    ...
    def forward(self, z, pos, batch=None):
        ...
        for convolution, fc, global_agg in zip(self.interaction_blocks, self.fc_blocks, self.global_blocks):
            if self.spherical_embedding:
                x = x + convolution(x, rbf, edge_index, sbf = sbf, idx_kj = idx_kj, idx_ji = idx_ji) # graph convolution with angular features
            else:
                x = x + convolution(x, rbf, edge_index) # graph convolution with radial features

            x = x + fc(x) # feature-wise 1D convolution

            x = x + global_agg(x,batch) # aggregate global information to all nodes
...
class GCBlock(torch.nn.Module):
    def __init__(self, graph_convolution_filters, hidden_channels, radial_dim, convolution_mode, spherical_dim = None, spherical = False, norm=None, dropout=0, heads = 1):
        ...
        self.message_to_node = nn.Linear(graph_convolution_filters,hidden_channels)
...
class MPConv(torch.nn.Module):
    def __init__(self, in_channels, out_channels, edge_dim):
        ...
        self.linear = nn.Linear(in_channels * 2 + edge_dim, out_channels, bias=False)


    def forward(self, x, edge_index, edge_attr):
        i, j = edge_index
        m = self.linear(torch.cat((x[i], x[j], edge_attr),dim=-1))

        # one of these-two is legit
        #return scatter(m,i,dim=0) # sometimes breaks with this one
        return scatter(m,j,dim=0)
