...
class EmbeddingBlock(torch.nn.Module):
    def __init__(self, num_radial, hidden_channels, num_atom_features, embedding_size, embedding_dimension, act=swish):
        ...
        for key in embedding_size.keys():
            self.embeddings.append(Embedding(embedding_size[key], embedding_dimension))
    ...
    def reset_parameters(self):
        [self.embeddings[i].weight.data.uniform_(-sqrt(3), sqrt(3)) for i in range(len(self.embeddings))]
...
class CustomEmbeddingBlock(torch.nn.Module):
    def __init__(self, hidden_channels, num_atom_features, embedding_dim, act=swish):
        ...
        self.emb2 = Embedding(embedding_dim, hidden_channels)
        self.lin = Linear(hidden_channels + (num_atom_features - 2), hidden_channels)

        self.reset_parameters()

    def reset_parameters(self):
        self.emb1.weight.data.uniform_(-sqrt(3), sqrt(3))
        self.emb2.weight.data.uniform_(-sqrt(3), sqrt(3))
        self.lin.reset_parameters()
...
class InteractionBlockPP(torch.nn.Module):
    def __init__(self, hidden_channels, graph_convolution_filters, num_spherical,
        ...
        self.lin_rbf1 = Linear(num_radial, num_radial, bias=False)
        self.lin_rbf2 = Linear(num_radial, hidden_channels, bias=False)
        self.lin_sbf1 = Linear(num_spherical * num_radial, num_spherical * num_radial, bias=False)
        self.lin_sbf2 = Linear(num_spherical * num_radial, int_hidden_channels, bias=False)
    ...
    def reset_parameters(self):
        ...
        glorot_orthogonal(self.up_1x1.weight, scale=2.0)
        glorot_orthogonal(self.down_1x1.weight, scale=2.0)
        glorot_orthogonal(self.lin_kj.weight, scale=2.0)
        self.lin_kj.bias.data.fill_(0)
        glorot_orthogonal(self.lin_ji.weight, scale=2.0)
        self.lin_ji.bias.data.fill_(0)
...
class CustomDimeNet(torch.nn.Module):
    ...
    def __init__(self, hidden_channels: int, graph_filters: int, out_channels: int,
                 num_blocks: int, num_spherical: int,
        ...
        self.atom_embeddings = EmbeddingBlock(num_radial, hidden_channels, num_atom_features, atom_embedding_dims, embedding_hidden_dimension, act)

        self.output_blocks = torch.nn.ModuleList([
            OutputBlockPP(num_radial, hidden_channels, out_channels,
                          num_output_layers, act) for _ in range(num_blocks + 1)
        ])

        self.interaction_blocks = torch.nn.ModuleList([
            InteractionBlockPP(hidden_channels, graph_filters, num_spherical,
    ...
    def reset_parameters(self):
        ...
        self.atom_embeddings.reset_parameters()
    ...
    def forward(self, z, pos, batch=None):
        ...
        # Embedding block.
        x = self.atom_embeddings(z, rbf, i, j)
        P = self.output_blocks[0](x, rbf, i, num_nodes=pos.size(0))

        # Interaction blocks.
        for interaction_block, output_block, norm, dropout in zip(self.interaction_blocks, self.output_blocks[1:], self.norms, self.dropouts):
