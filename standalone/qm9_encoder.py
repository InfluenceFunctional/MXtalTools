...
from crystal_building.utils import random_crystaldata_alignment
from models.autoencoder_models import point_autoencoder
import numpy as np
import torch

from models.utils import compute_full_evaluation_overlap, reload_model

config_path = '/standalone/qm9_encoder.yaml'
checkpoint_path = '/cluster/best_autoencoder_autoencoder_tests_qm9_test21_20_13-02-10-13-47'
...
class Qm9Autoencoder(torch.nn.Module):
    def __init__(self,
        ...
        self.model = point_autoencoder(seed=12345, config=self.config.autoencoder.model, num_atom_types=num_atom_types)
        for param in self.model.parameters():  # freeze encoder
            param.requires_grad = False
        self.model = reload_model(self.model, optimizer=None, path=checkpoint_path)
        self.model.eval()
    ...
    def forward(self, data):
        with torch.no_grad():
            if self.model.variational:
                return self.model.encode(data, z=torch.zeros((data.num_graphs, 3,  # uniform prior for comparison/inference
    ...
    def evaluate_encoding(self, data, return_encodings=False):
        with torch.no_grad():
            encoding = self.forward(data.clone())
