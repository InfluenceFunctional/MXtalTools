...
class NSF_CL(nn.Module):
    ...
    def __init__(self, dim, K=5, B=3, hidden_dim=8, base_network=MLP, conditioning_dim = 0):
        super().__init__()
        self.dim = dim
        self.K = K
        self.B = B
        self.conditioning_dim = conditioning_dim
        self.f1 = base_network(dim // 2 + conditioning_dim, (3 * K - 1) * dim // 2, hidden_dim)
        self.f2 = base_network(dim // 2 + conditioning_dim, (3 * K - 1) * dim // 2, hidden_dim)

    def forward(self, x):
        # set conditioning
        if self.conditioning_dim > 0:
            conditions = x[:,-self.conditioning_dim:]
            x = x[:,:-self.conditioning_dim]
        ...
        # run conditioner for lower channel
        if self.conditioning_dim > 0:
            out = self.f1(torch.cat((lower,conditions),dim=1)).reshape(-1, self.dim // 2, 3 * self.K - 1)
        else:
            out = self.f1(lower).reshape(-1, self.dim // 2, 3 * self.K - 1)

        # do the spline transformation
        ...
        # run conditioner for upper channel
        if self.conditioning_dim > 0:
            out = self.f2(torch.cat((upper,conditions),dim=1)).reshape(-1, self.dim // 2, 3 * self.K - 1)
        else:
    ...
    def backward(self, z):
        # set conditions
        if self.conditioning_dim > 0:
            conditions = z[:,-self.conditioning_dim:]
            z = z[:,:-self.conditioning_dim]

        # initialize the log-det
        log_det = torch.zeros(z.shape[0]).to(z.device)

        # split channels
        lower, upper = z[:, :self.dim // 2], z[:, self.dim // 2:]

        # run the conditioner for the upper channel
        if self.conditioning_dim > 0:
            out = self.f2(torch.cat((upper,conditions),dim=1)).reshape(-1, self.dim // 2, 3 * self.K - 1)
        else:
            out = self.f2(upper).reshape(-1, self.dim // 2, 3 * self.K - 1)
        ...
        # run the conditioner for the lower channel
        if self.conditioning_dim > 0:
            out = self.f1(torch.cat((lower,conditions),dim=1)).reshape(-1, self.dim // 2, 3 * self.K - 1)
        else:
            out = self.f1(lower).reshape(-1, self.dim // 2, 3 * self.K - 1)
