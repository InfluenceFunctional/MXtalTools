...
batch_sizes = [1000]
norms = [None]
electrostatic_scaling_factors = [0, 1000, 10000]
...
default_config = {
    ...
    'weight_decay': 0.0001,
    'device': 'cuda',
    'electrostatic_scaling_factor': 1000,
    }  # 0 - baseline
...
for l in layers:
    for f in filters:
        for b in batch_sizes:
            for norm in norms:
                for electrostatic_scaling_factor in electrostatic_scaling_factors:
                    for embed in embedding_types:
                        ...
                        config['norm'] = norm
                        config['electrostatic_scaling_factor'] = electrostatic_scaling_factor
